from transformers import AutoTokenizer, AutoModelForMultipleChoice, TrainingArguments, Trainer, AutoModel
...
# model = AutoModelForMultipleChoice.from_pretrained(f"{model_name}_trained_model")
model = MultipleChoiceModel()
model.load_state_dict(torch.load(f'{model_name}_trained_model/{model_name}_trained_model.pt'))
model.to(device)
...
test_dataloader = DataLoader(encoded_dataset["test"], batch_size=3)


args = TrainingArguments(
    f"{model_name}_train_bk",
...
def get_predictions(dataloader, model):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader):
            ...
            X = torch.cat(X, dim=1).to(device) # torch.Size([4, 3, batch, MAX_LEN])
            pred = model(X)
...
np.savetxt(f"./{model_name}_trained_model/{model_name}_prediction.csv", preds, fmt="%s")
